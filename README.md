...
2. Use [emscripten](https://github.com/kripken/emscripten) to compile the C code into a web binary. 

Use `emcc file.c -o file.js` to create node.js runnable JavaScript. Use `emcc file.c -o file.html` to create something that can be pulled up in a web browser. 

Just like `"use strict"`, some browsers also support `"use asm"` if they support the speedier optimizations. This can be used to get to the more optimized JavaScript. The syntax looks foreign but it is using the asm.js optimizations to create very machine friendly code!
